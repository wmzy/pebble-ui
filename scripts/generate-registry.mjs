...
const localImportClosure = (rootFiles) => {
  const seen = new Set();
...
const wrapper = ({ components, types, css }) => {
  ...
  const cssImports = css === TOKENS_FAMILY ? [TOKENS_FAMILY] : [TOKENS_FAMILY, css];
  const typeExport =
...
for (const [family, entry] of [...families].sort(([a], [b]) => a.localeCompare(b))) {
  ...
  items.push({
    ...
    content: wrapper({ components: entry.exports, types: entry.types, css: family }),
  });
}
